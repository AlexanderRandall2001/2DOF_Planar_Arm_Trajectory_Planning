...
def joint_step(start_thetas, target_thetas, i, steps, mode):
    ...
    # Interpolate end-effector position in joint space

    return [
        start + h * (target - start)
        for start, target in zip(start_thetas, target_thetas)
    ]

# Full joint-space trajectory

def trajectory_joint_space(start_thetas, target, lengths, steps, mode="cubic"):

    # Compute elbow up target joint angles from IK

    target_thetas = ik_2dof(target, lengths)[1]
    traj = []

    # Generate trajectory step-by-step

    for i in range(steps + 1):
        traj.append(joint_step(start_thetas, target_thetas, i, steps, mode))
